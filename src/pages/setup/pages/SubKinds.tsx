...
const SubKinds = () => {
    ...
    const [formData, setFormData] = useState<Partial<SubKindData>>({
        name: '',
        description: '',
        status: 'active',
        kind_id: '',
    ...
    const { data: subKinds = [], isLoading, isError, error } = useGetAllSubKinds();
    const { data: kinds = [] } = useGetAllKinds();
    const createMutation = useCreateSubKind();
    const updateMutation = useUpdateSubKind();
    const deleteMutation = useDeleteSubKind();

    // Show error toast if query fails
    useEffect(() => {
        if (isError) {
            Swal.fire('Error', error?.message || 'Failed to fetch sub-kinds', 'error');
        }
    }, [isError, error]);

    // Helper function to get kind name
    const getKindName = (kindId: string) => {
    ...
    const filteredRecords = useMemo(() => {
        if (!search) return subKinds;

        return subKinds.filter((item) => {
            const kindName = getKindName(item.kind_id);
            return Object.keys(item).some((key) => {
                return String(item[key as keyof SubKindData]).toLowerCase().includes(search.toLowerCase());
            }) || kindName.toLowerCase().includes(search.toLowerCase());
        });
    }, [subKinds, search, kinds]);
    ...
    const handleAdd = () => {
        setIsEdit(false);
        setFormData({ name: '', description: '', status: 'active', kind_id: '' });
        setShowModal(true);
    };

    const handleEdit = (record: SubKindData) => {
        setIsEdit(true);
        setFormData(record);
        setShowModal(true);
    };
    ...
    const handleDelete = async (id: string) => {
        Swal.fire({
            title: '<span style="color: #dc2626; font-size: 20px; font-weight: 600;">Delete Sub-Kind?</span>',
            html: `
        <div style="text-align: center;">
          ...
          <p style="color: #6b7280; font-size: 14px; margin-top: 8px;">This action cannot be undone. The sub-kind will be permanently removed.</p>
        </div>
      `,
            icon: undefined,
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    Swal.fire({
                        ...
                        html: '<p style="color: #6b7280; font-size: 14px;">Sub-kind has been deleted successfully.</p>',
                        icon: 'success',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.kind_id) {
            Swal.fire('Error', 'Please select a kind', 'error');
            return;
        }
        ...
        try {
            if (isEdit && formData.$id) {
                ...
                Swal.fire('Updated!', 'Sub-kind has been updated.', 'success');
            } else {
                ...
                Swal.fire('Created!', 'Sub-kind has been created.', 'success');
            }
            setShowModal(false);
        } catch (error: any) {
    ...
    return (
        <div className="panel">
            <div className="flex md:items-center md:flex-row flex-col mb-5 gap-5">
                <h5 className="font-semibold text-lg dark:text-white-light">Sub-Kinds</h5>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-3xl max-h-[75vh] overflow-hidden">
                        ...
                        <div className="bg-primary px-6 py-3 border-b border-gray-200 dark:border-gray-700">
                            <h3 className="text-lg font-semibold text-white">{isEdit ? 'Edit' : 'Add'} Sub-Kind</h3>
                        </div>
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="p-4 space-y-1 max-h-[calc(75vh-140px)] overflow-y-auto">
                                <div className="flex items-center gap-4 py-3 border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 px-2 rounded">
                                    <label className="w-36 font-semibold text-gray-700 dark:text-gray-300">Kind:</label>
                                    <select
                                        className="form-select flex-1"
                                        value={formData.kind_id}
                                        onChange={(e) => setFormData({ ...formData, kind_id: e.target.value })}
                                        required
                                    >
                                        <option value="">Select Kind</option>
                                        {kinds.filter(k => k.status === 'active').map((kind) => (
