...
import { type MachineryTypeResponse } from '../services/machineryTypes';
import {
    useGetAllMachineryTypes,
    useCreateMachineryType,
    useUpdateMachineryType,
    useDeleteMachineryType,
} from '../hooks/useMachineryTypes';

type MachineryTypeData = MachineryTypeResponse;
...
const MachineryTypes = () => {
    ...
    const [formData, setFormData] = useState<Partial<MachineryTypeData>>({
        name: '',
        description: '',
        status: 'active',
    });
    ...
    const { data: machineryTypes = [], isLoading, isError, error } = useGetAllMachineryTypes();
    const createMutation = useCreateMachineryType();
    const updateMutation = useUpdateMachineryType();
    const deleteMutation = useDeleteMachineryType();

    // Show error toast if query fails
    useEffect(() => {
        if (isError) {
            Swal.fire('Error', error?.message || 'Failed to fetch machinery types', 'error');
        }
    }, [isError, error]);
    ...
    const columns = [
        ...
        },
        {
            ...
            title: 'Status',
            sortable: true,
            render: ({ status }: MachineryTypeData) => (
        ...
        },
        {
            ...
            title: 'Created At',
            sortable: true,
            render: ({ $createdAt }: MachineryTypeData) => (
        ...
        },
        {
            ...
            render: (record: MachineryTypeData) => (
    ...
    const filteredRecords = useMemo(() => {
        if (!search) return machineryTypes;

        return machineryTypes.filter((item) => {
            return Object.keys(item).some((key) => {
                return String(item[key as keyof MachineryTypeData]).toLowerCase().includes(search.toLowerCase());
            });
        });
    }, [machineryTypes, search]);
    ...
    const handleEdit = (record: MachineryTypeData) => {
        setIsEdit(true);
        setFormData(record);
        setShowModal(true);
    };

    const handleView = (record: MachineryTypeData) => {
        ...
        Swal.fire({
            title: `<strong style="color: ${isDark ? '#60a5fa' : '#4361ee'};">Machinery Type Details</strong>`,
            html: `
    ...
    const handleDelete = async (id: string) => {
        Swal.fire({
            title: '<span style="color: #dc2626; font-size: 20px; font-weight: 600;">Delete Machinery Type?</span>',
            html: `
        <div style="text-align: center;">
          ...
          <p style="color: #6b7280; font-size: 14px; margin-top: 8px;">This action cannot be undone. The machinery type will be permanently removed.</p>
        </div>
      `,
            icon: undefined,
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    Swal.fire({
                        ...
                        html: '<p style="color: #6b7280; font-size: 14px;">Machinery type has been deleted successfully.</p>',
                        icon: 'success',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isEdit && formData.$id) {
                ...
                Swal.fire('Updated!', 'Machinery type has been updated.', 'success');
            } else {
                ...
                Swal.fire('Created!', 'Machinery type has been created.', 'success');
            }
            setShowModal(false);
        } catch (error: any) {
    ...
    return (
        <div className="panel">
            <div className="flex md:items-center md:flex-row flex-col mb-5 gap-5">
                <h5 className="font-semibold text-lg dark:text-white-light">Machinery Types</h5>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-3xl max-h-[75vh] overflow-hidden">
                        ...
                        <div className="bg-primary px-6 py-3 border-b border-gray-200 dark:border-gray-700">
                            <h3 className="text-lg font-semibold text-white">{isEdit ? 'Edit' : 'Add'} Machinery Type</h3>
                        </div>
