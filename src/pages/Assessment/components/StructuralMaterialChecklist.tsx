...
const StructuralMaterialChecklist: React.FC<StructuralMaterialChecklistProps> = ({ onInputChange }) => {
    ...
    const handleAddFloor = (section: string) => {
        setAdditionalFloors(prev => {
            const nextFloor = (prev[section]?.length > 0
                ? Math.max(...prev[section])
                : 4) + 1;
            return {
                ...prev,
                [section]: [...(prev[section] || []), nextFloor]
            };
        });
    };
                        ...
                        <tr>
                            ...
                            <td>
                                <div className="flex items-center gap-4 pl-[30px]">
                                    ...
                                    {Array.from(new Set(Object.values(additionalFloors).flat())).sort((a, b) => a - b).map((floorNum) => (
                                        <div key={floorNum} className="w-4 text-center text-xs font-medium">{floorNum}th</div>
                                    ))}
                                </div>
                            </td>
                        </tr>
