...
/**
 * Classifications Tables Appwrite Setup Script
 * Creates two related collections:
 * 1. classifications
 * 2. sub_classifications
 * 
...
console.log('🚀 Starting Classifications Tables Setup with Admin API Key...');
...
class ClassificationsTablesSetup {
    constructor() {
    ...
    async setupClassifications() {
        const collectionId = 'classifications';
        ...
        // Create collection
        await this.createCollection(collectionId, 'Classifications');
        ...
        console.log('✅ Classifications setup complete!');
    }

    async setupSubClassifications() {
        const collectionId = 'sub_classifications';
        
        console.log('\n📂 === SETTING UP SUB-CLASSIFICATIONS ===');
        ...
        // Create collection
        await this.createCollection(collectionId, 'Sub-Classifications');
        await this.delay(1000);
        ...
        await this.createStringAttribute(collectionId, 'classification_id', 100, true);
        await this.delay(300);

        // Create indexes
        console.log('\n📇 Creating indexes...');
        
        await this.createIndex(collectionId, 'classification_id_fk_index', 'key', ['classification_id']);
        await this.delay(500);
        ...
        console.log('✅ Sub-Classifications setup complete!');
    }
    ...
    async setupAllTables() {
        try {
            ...
            console.log('\n🎯 Starting setup of all classification tables...\n');

            // Setup tables in order (parent tables first)
            await this.setupClassifications();
            await this.delay(2000);
            
            await this.setupSubClassifications();

            console.log('\n🎉 ALL CLASSIFICATION TABLES SETUP COMPLETED SUCCESSFULLY!');
            console.log('\n📋 Summary:');
            ...
            console.log('   - Attributes: $id (PK - auto), name, description, status, classification_id (FK)');
            console.log('   - Indexes: classification_id_fk_index, status_index, name_index');
            console.log('');
            console.log('🔗 Relationships:');
            console.log('   classifications.$id → sub_classifications.classification_id (1:N)');
            console.log('');
            console.log('🚀 You can now start using these tables!');
            console.log('\n💡 Next steps:');
            console.log('   1. Add to .env file:');
            console.log('      VITE_APPWRITE_CLASSIFICATIONS_COLLECTION_ID=classifications');
            console.log('      VITE_APPWRITE_SUB_CLASSIFICATIONS_COLLECTION_ID=sub_classifications');
...
// Run the setup
const setup = new ClassificationsTablesSetup();
