...
/**
 * Building Depreciation Tables Appwrite Setup Script
 * Creates three related collections:
 * 1. building_structural_types
 * 2. building_codes
 * 3. building_depreciation
 * 
...
console.log('🚀 Starting Building Depreciation Tables Setup with Admin API Key...');
...
class DepreciationTablesSetup {
    ...
    async createIntegerAttribute(collectionId, key, required = false, defaultValue = 0) {
        const url = `${this.endpoint}/databases/${this.databaseId}/collections/${collectionId}/attributes/integer`;
    ...
    async setupBuildingStructuralTypes() {
        const collectionId = 'building_structural_types';
        
        console.log('\n🏗️ === SETTING UP BUILDING STRUCTURAL TYPES ===');
        ...
        // Create collection
        await this.createCollection(collectionId, 'Building Structural Types');
        ...
        await this.createStringAttribute(collectionId, 'description', 1000, false);
        await this.delay(300);
        ...
        console.log('✅ Building Structural Types setup complete!');
    }

    async setupBuildingCodes() {
        const collectionId = 'building_codes';
        
        console.log('\n🏢 === SETTING UP BUILDING CODES ===');
        ...
        // Create collection
        await this.createCollection(collectionId, 'Building Codes');
        await this.delay(1000);
        ...
        console.log('✅ Building Codes setup complete!');
    }

    async setupBuildingDepreciation() {
        const collectionId = 'building_depreciation';
        
        console.log('\n📉 === SETTING UP BUILDING DEPRECIATION ===');
        ...
        // Create collection
        await this.createCollection(collectionId, 'Building Depreciation');
        await this.delay(1000);
        ...
        await this.createIntegerAttribute(collectionId, 'age', false, 0);
        await this.delay(300);
        ...
        // Foreign keys
        await this.createStringAttribute(collectionId, 'building_structural_types_id', 50, false);
        await this.delay(300);
        
        await this.createStringAttribute(collectionId, 'building_code_id', 50, false);
        await this.delay(300);
        ...
        await this.createIndex(collectionId, 'age_index', 'key', ['age']);
        await this.delay(500);
        
        await this.createIndex(collectionId, 'rate_index', 'key', ['rate']);
        await this.delay(500);
        
        await this.createIndex(collectionId, 'structural_types_fk_index', 'key', ['building_structural_types_id']);
        await this.delay(500);
        
        await this.createIndex(collectionId, 'building_code_fk_index', 'key', ['building_code_id']);
        await this.delay(500);
        
        await this.createIndex(collectionId, 'status_index', 'key', ['status']);
        await this.delay(500);

        console.log('✅ Building Depreciation setup complete!');
    }
    ...
    async setupAllTables() {
        try {
            ...
            console.log('\n🎯 Starting setup of all depreciation tables...\n');

            // Setup tables in order (parent tables first)
            await this.setupBuildingStructuralTypes();
            await this.delay(2000);
            
            await this.setupBuildingCodes();
            await this.delay(2000);
            
            await this.setupBuildingDepreciation();

            console.log('\n🎉 ALL DEPRECIATION TABLES SETUP COMPLETED SUCCESSFULLY!');
            console.log('\n📋 Summary:');
            console.log('✅ building_structural_types:');
            console.log('   - Attributes: name (PK), description, status');
            console.log('   - Indexes: name_index, status_index');
            console.log('');
            console.log('✅ building_codes:');
            console.log('   - Attributes: name (PK), description, status');
            console.log('   - Indexes: name_index, status_index');
            console.log('');
            console.log('✅ building_depreciation:');
            console.log('   - Attributes: name, age, rate, effectivity_date, status');
            console.log('   - Foreign Keys: building_structural_types_id, building_code_id');
            console.log('   - Indexes: name_index, age_index, rate_index, structural_types_fk_index, building_code_fk_index, status_index');
            console.log('');
            console.log('🔗 Relationships:');
            console.log('   building_structural_types → building_depreciation (1:N)');
            console.log('   building_codes → building_depreciation (1:N)');
            console.log('');
...
// Run the setup
const setup = new DepreciationTablesSetup();
