...
/**
 * Products Table Appwrite Setup Script
 * Creates the products collection with:
 * - product_id (PK, auto-generated)
 * - name
 * - description
 * - status
 * 
...
console.log('🚀 Starting Products Table Setup with Admin API Key...');
...
class ProductsTableSetup {
    ...
    async setupProducts() {
        ...
        // Create collection
        await this.createCollection(collectionId, 'Products');
        await this.delay(1000);

        // Create attributes
        console.log('\n📝 Creating attributes...');
        
        // Note: product_id is auto-generated as $id by Appwrite
        console.log('  ℹ️ product_id will be auto-generated as $id (Primary Key)');
        
        await this.createStringAttribute(collectionId, 'name', 500, true);
        await this.delay(300);
        
        await this.createStringAttribute(collectionId, 'description', 2000, false);
        await this.delay(300);
        ...
        console.log('   - Fields: name, description, status');
        console.log('   - Auto Fields: $id, $createdAt, $updatedAt');
        console.log('\n💡 Add this to your .env file:');
        console.log('   VITE_APPWRITE_PRODUCTS_COLLECTION_ID=products');
    }

    async run() {
        try {
            await this.setupProducts();
            console.log('\n🎉 All done! Products table is ready to use.');
        } catch (error) {
...
// Run the setup
const setup = new ProductsTableSetup();
setup.run();
